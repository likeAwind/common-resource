...
import com.windforce.common.resource.other.Getter;
import com.windforce.common.resource.other.GetterBuilder;
import com.windforce.common.resource.other.IndexGetter;
import com.windforce.common.resource.other.InjectDefinition;
...
public class Storage<K, V> extends Observable implements ApplicationContextAware {
	...
	public synchronized void initialize(ResourceDefinition definition) {
		...
		for (InjectDefinition inject : injects) {
			...
			try {
				...
				FormattingTuple message = MessageFormatter.format("无法注入静态资源[{}]的[{}]属性值", definition.getClz().getName(),
						inject.getField().getName());
	 ...
	 *            键
	 * @param flag
	 *            不存在时是否抛出异常,true:不存在时抛出异常,false:不抛出异常返回null
	 * @return
	 */
	...
	public void reload() {
		...
		try {
			...
			if (resourceDefinition.getCacheKey() != null) {
				InputStream inputStream = StorageManager.getFromCache(resourceDefinition.getCacheKey());
				if (inputStream != null) {
					input = inputStream;
				} else {
	...
	private V put(V value) {
		...
		for (IndexGetter getter : indexGetters.values()) {
			...
			/*
			 * 2012.2.28 改为允许null值作为索引值(可稍微简化静态类设计) // 无效索引值检查 if (indexKey == null) {
			 * FormattingTuple message = MessageFormatter.format("[{}]资源的索引[{}]的值为 null",
			 * getClz().getName(), name); logger.debug(message.getMessage()); continue; }
			 */
			// 索引内容存储
			if (getter.isUnique()) {
